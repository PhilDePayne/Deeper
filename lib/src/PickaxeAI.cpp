...
void PickaxeAI::pickaxeThrow(int dir, int orientation, int reverse, float deltaTime) {
	...
	if (distance > 0 || firstTravel) {
		if (orientation == 1)
			parent->getComponent<Model>(ComponentType::MODEL)->transform.position.x += reverse * dir * 1.2 * glm::sin(2 * (glfwGetTime() - throwTime));
		else
			parent->getComponent<Model>(ComponentType::MODEL)->transform.position.z += reverse * -dir * 1.2 * glm::sin(2 * (glfwGetTime() - throwTime));
