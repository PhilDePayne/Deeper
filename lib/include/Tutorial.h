...
public:
    ...
    void loadResources(int display_w, int display_h, Shader shader) {
        ...
        info = {move, rotate, pickThrow, pickGrab};
    }
    ...
    void display(GLFWwindow * window, Shader shader) {
        ...
        if ((glfwGetKey(window, GLFW_KEY_A) == GLFW_PRESS || glfwGetKey(window, GLFW_KEY_D) == GLFW_PRESS) && n == 0)
            n++;
        else if((glfwGetKey(window, GLFW_KEY_LEFT) == GLFW_PRESS || glfwGetKey(window, GLFW_KEY_RIGHT) == GLFW_PRESS) && n == 1)
            n++;
        else if(glfwGetKey(window, GLFW_KEY_F) == GLFW_PRESS && n == 2)
            n++;
        else if(glfwGetKey(window, GLFW_KEY_G) == GLFW_PRESS && n == 3)
            n++;
